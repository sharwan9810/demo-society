...
$query = " select * from loans where loanstatus='APPROVED'";
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <?php 
                                    ...
                                    while($row=mysqli_fetch_assoc($result))
                                    {
                                        ...
                                        $lan = $row['lan'];
                                        $UserName = $row['fname'];
                                        $UserAadhar = $row['accountno'];
                                        $UserEmail = $row['pannumber'];
                                        $UserAge = $row['phonenumber'];
                                        $date = $row['statusdate'];
                                        $status = $row['ldcr'];
                                        ?>
...
$sql = "SELECT  SUM(ldcr-lddr)+SUM(ldcr-lddr)*1.25/100 from newloan";
$result = $conn->query($sql);
//display data on web page
while($row = mysqli_fetch_array($result)){
$row['SUM(ldcr-lddr)+SUM(ldcr-lddr)*1.25/100'];
echo  "<td>". $row['SUM(ldcr-lddr)+SUM(ldcr-lddr)*1.25/100']."</td>";
}
